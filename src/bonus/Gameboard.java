	...
	private void generateFeedback(String guess)
	{
		...
		for(int i = 0; i<guess.length(); i++)
		{
			if(codeString.charAt(i) == guess.charAt(i))
			{
				...
				tempCodeString = tempCodeString.substring(0,i) + "N" + tempCodeString.substring(i+1);
				tempGuess = tempGuess.substring(0,i) + "n" + tempGuess.substring(i+1);
			}
		}
		//generate white pegs
		for(int i = 0; i<tempGuess.length(); i++)
		{
			for(int j = 0; j<tempCodeString.length(); j++)
			{
				if(tempGuess.charAt(i) == tempCodeString.charAt(j))
				{
					...
					tempCodeString = tempCodeString.substring(0,j) + "N" + tempCodeString.substring(j+1);
				}
			}
		}
